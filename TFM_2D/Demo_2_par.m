...
t = tic;
data(:, :, :, 1) = Forward2D(squeeze(Points(1, :, :)), image, geo);
data(:, :, :, 2) = Forward2D(squeeze(Points(2, :, :)), image, geo);
toc(t);
...
t = tic;
recon_1 = TFD2D(squeeze(Points(1, :, :)), data(:, :, :, 1), geo);
recon_2 = TFD2D(squeeze(Points(2, :, :)), data(:, :, :, 2), geo);
