...
%                 Points(jj, 3) = distance_2(fix((jj - 1) / size(distance_1, 2)) + 1);
    ...
    case '2_parallel'
        Points = zeros(2, size(distance_1, 2), 3);
        for jj = 1:size(distance_1, 2) * size(distance_2, 2)
                Points(1, jj, 1) = 0;
                Points(1, jj, 2) = distance_1(mod((jj - 1), size(distance_1, 2)) + 1);
                Points(1, jj, 3) = distance_2(fix((jj - 1) / size(distance_1, 2)) + 1);
                Points(2, jj, 1) = geo.sx;
                Points(2, jj, 2) = distance_1(mod((jj - 1), size(distance_1, 2)) + 1) ;
                Points(2, jj, 3) = distance_2(fix((jj - 1) / size(distance_1, 2)) + 1);
        end
