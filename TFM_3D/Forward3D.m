function [e_t] = Forward3D(Points, image, geo)
        ...
        for ii = 1:size(image, 1)
            for jj = 1:size(image, 2)
                for kk = 1:size(image, 3)
                    if(image(ii, jj, kk) ~= 0)
                        d_t = sqrt((ii * geo.dx - Points(mm, 1))^2 + (jj * geo.dy - Points(mm, 2)) ^ 2 + (kk * geo.dz - Points(mm, 3)) ^ 2);
                        d_r = sqrt((ii * geo.dx - Points(nn, 1))^2 + (jj * geo.dy - Points(nn, 2)) ^ 2 + (kk * geo.dz - Points(mm, 3)) ^ 2);
                        t = (d_t + d_r) / geo.c;
                        t_idx = ceil(t * geo.f);
                        e_t(mm, nn, t_idx) = e_t(mm, nn, t_idx) + 1;
%                         w = 2 * pi * geo.f * (1:geo.L) / geo.L;
%                         k = w ./ geo.c;
%                         E_w = E_w + image(ii, jj, kk) * (-1 ./ (8 * pi * sqrt(d_t * d_r) * k)) .* exp(-1i * k * (d_t + d_r));
                    end
                end
            end
        end
